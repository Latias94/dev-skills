...
class PlannerHookAdapterTests(unittest.TestCase):
    def test_ready_repo_emits_hook_payload_and_wrapped_prompt(self) -> None:
        with create_repo() as tmp:
            ...
            self.assertEqual(payload["hook_payload"]["hookSpecificOutput"]["hookEventName"], "UserPromptSubmit")
            self.assertIn("<planner-runtime>", payload["hook_payload"]["hookSpecificOutput"]["additionalContext"])
            self.assertIn("<user-request>", payload["wrapped_prompt"])

    def test_audit_repo_emits_audit_hook_payload(self) -> None:
        with create_repo() as tmp:
            root = Path(tmp)
            ...
            result = run_script(
                root,
                "Explain whether anything is assignable or if this stays audit-only.",
                "--event-name",
                "BeforeAgent",
            )
            payload = json.loads(result.stdout)
            ...
            self.assertEqual(payload["hook_payload"]["hookSpecificOutput"]["hookEventName"], "BeforeAgent")
            self.assertIn(
                "Stay read-only. Do not fabricate worker dispatch",
                payload["hook_payload"]["hookSpecificOutput"]["additionalContext"],
            )
