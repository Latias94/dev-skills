...
大型 program 编排使用明确状态机：
`DISCOVERY -> SHAPE -> PLAN -> ASSIGN -> EXECUTE -> INTAKE -> REVIEW -> VERIFY -> INTEGRATE`。
`RECON` 用于只读探索下一波候选；`DECISION` 用于无法安全推断的产品、ADR、cross-repo 或
side-effect tradeoff。

大型仓库分配 implementation 前，先跑只读状态和校验脚本：
...
`Implementation Horizon: 0` 表示当前没有可分配的 active task 或 campaign；先修复 docs、
context、gates、task state 或 campaign state。README 只负责 discoverability，不是任务事实源。
