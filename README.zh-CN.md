...
创建 workstream 或 bundle 前，`$plan-architecture-lane` 帮 Planner 选择 planning depth：
...
workstream 新建一个 worktree。Planner 负责提出终端/worktree 布局、分支名、创建命令和
提示词；用户批准后，Planner 可以帮忙创建，或把命令交给用户执行。Lane 终端完成任务后
推荐同 lane 下一步，但全局顺序仍由 Planner 负责。

长期运行的终端应该先由 Planner 准备 **lane goal bundle**：一个 lane、一个稳定 worktree、
...
有边界任务，或 Planner 批准的 lane campaign；不适合作为整个 architecture lane 的无限目标。
当 bundle、campaign 或 task 已经适合较长时间自动执行时，Planner 输出应包含要设置的精确 Codex goal，
...
workstreams、validation ladder、shared scopes 和 next bundles。队列变薄时，Planner 应该用
...
已有 TODO。如果已经有足够多的有序 bundles ready，Planner 可以批准 lane campaign，让 goal 按
gate 和 checkpoint 连续跑多个 bundle 后再请求输入。
分配 worker 或 lane 终端后，Planner 也可以用空档做只读 architecture reconnaissance：对全仓库或
...
| 大型仓库、有稳定能力域 | `$audit-project-scale`，再 `$run-architecture-lane` | 稳定 lane worktree，加 planner 批准创建 |
...
把 Planner 批准的 lane bundle 或 lane campaign 作为最大自主范围。
...
对于 active 或 stale worktrees，把 session-tail helper 作为补充上下文，先不要让我手动复制聊天。
...
给 planner 批准的 lane bundle 设置 Codex goal：

```text
把 planner state 里的 lane bundle storage-20260530-01 设置为当前 Codex goal。
保持在 bundle scope 内；遇到 shared-scope 变更或缺失 context 时停止，并回报 Planner。
```

给 Planner 批准的 lane campaign 设置 Codex goal：
