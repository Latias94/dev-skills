...
- Owned scopes can move inside the lane; shared scopes require upper-planner coordination.
- Close and verify the active workstream before starting the next queued workstream.
- A goal bundle is one approved unit of lane work: task IDs, context manifest, validation,
...
- If requirements and gates are clear, an upper planner may define an ordered lane campaign so one terminal
