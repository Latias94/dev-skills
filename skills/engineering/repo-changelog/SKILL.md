...
1. Establish the release contract.
   ...
   - Confirm the output target, `BASE`, `TARGET`, version, date, and audience. Infer a missing base only when one reachable tag is clearly correct; prereleases normally compare with the preceding prerelease. Ask before drafting when refs fail, multiple bases remain plausible, or `BASE` is not an ancestor of `TARGET`.
   ...
   - Completion: both refs resolve, the intended range is explicit, and the target format is identified.
...
2. Build an outcome ledger from evidence.
   ...
   - Record each candidate as `outcome | audience | impact/action | refs | contributor | section | disposition`. Use a verified merged PR as the accounting unit; inspect individual commits when no PR owns them or the final state remains ambiguous.
   - Read [Evidence and attribution](references/evidence-and-attribution.md) when the range has merge noise, GitHub PRs/issues, external contributors, or security work.
...
3. Consolidate the final state.
   - Merge commits and PRs that produce one user outcome. Combine their verified references and discard intermediate, reverted, superseded, duplicate, and net-zero states.
   - Include internal refactors, tests, CI, and dependency work only when their final effect changes security, compatibility, reliability, performance, packaging, or a developer-facing workflow.
...
5. Verify before finishing.
   ...
   - Scan the whole release section for semantic repetition, empty headings, raw commit narration, internal jargon, manual wrapping, and accidentally disclosed private security details.
