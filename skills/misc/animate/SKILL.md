---
name: animate
description: Build an animation from scratch, making the decisions in the order that determines whether it feels right — should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation. Use when asked to animate something, add motion, make a component feel alive, or build a transition. For critiquing existing motion use review-animations; for a codebase-wide audit use the repository's established planning workflow with review-animations as the motion-quality lens.
---

# Building Animations

A construction skill. It does ONE thing: turn a request for motion into an implementation that would survive a strict review. It does not audit a codebase, critique a diff (that's `review-animations`), or hunt for places that could animate.
...
If the task needs a *component* rather than an animation - a toast, a drawer, a command menu, or a dropdown - stop and use the project's established accessible component library. If none exists, research a maintained accessible library before implementation. Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.
