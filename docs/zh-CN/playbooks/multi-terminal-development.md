...
使用 Program Action mode DISCOVERY、SHAPE、PLAN 或 ASSIGN。
```
...
使用 Program Action mode RECON、ASSIGN 或 DECISION。如果下一步是接受完成输出，切到 $integrate-lane-results。
```
...
- auto-advance rule；
- 每一步后的 checkpoint；
- side-effect policy（`manual`、`auto-commit`、`auto-commit-sync` 或 `auto-commit-sync-merge`）；
...
每个 campaign 都应该包含明确 side-effect policy。只预批本地提交时用 `auto-commit`。
稳定 lane 工作优先用 `auto-commit-sync`：在已接受 task/bundle 边界自动 commit，并在 clean gates 后把 main 同步回 lane worktree。
