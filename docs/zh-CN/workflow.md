...
并连续推进该能力域下的一组 workstreams。长期 lane 终端应该接收 Planner 批准的
...
- Lane goal bundle 是 Planner 的本地 / 运行态指派：task IDs、scope、context manifest、
...
- **Lane campaign**：Planner 批准的一组有序同 lane bundles 或 workstreams，可在一个更长 Codex goal 下按 checkpoint 和 stop conditions 执行。
...
Planner 输出应包含已批准 task、lane bundle 或 lane campaign 要设置的 Codex goals，而不是整个 architecture lane 的 goal。
当某条 lane 要持续成熟时，Planner 先刷新 lane backlog，再继续分配任务；Codex goal 仍然只绑定下一个有边界 bundle 或批准的 campaign。
...
9. 长期 lane 终端继续前，Planner 先准备 lane goal bundle 或 lane campaign。
