...
当目标是宽泛产品方向，需要 bounded grill、MVP ladder、capability map、architecture lanes、
优先级或 ADR candidates 后才能开 workstream 时，使用 `$shape-product-architecture`。
...
| 文档 | 什么时候更新 | 负责人 |
| --- | --- | --- |
| Product docs | 产品方向、参考产品压力、MVP 阶段、client surfaces、non-goals 或产品优先级变化 | bounded grill 后由 product architecture / planner 角色处理 |
...
- **Product architecture shaping**：把宽泛产品愿景变成 vision、MVP ladder、capability map、lane map、priority classes 和 ADR candidates。
...
5. 产品阶段、能力边界或 MVP 优先级不清楚时，使用 `$shape-product-architecture`。
6. 用户选定子架构方向且还未创建 workstream 时，使用 `$plan-architecture-lane`。
7. 大功能和重构由 `$dev-flow` 委托给 `$open-workstream`。
8. 一个终端需要长期负责某个能力域时，使用 `$run-architecture-lane`。
9. 多终端活跃时，上层架构终端使用 `$plan-engineering-program`。
10. 已完成的 lane / worker 输出需要进入 review、verify、merge 或 sync 时，使用 `$integrate-lane-results`。
11. 可执行切片由 `$run-workstream-task` 委托给 `$tdd` 或 `$diagnose`。
